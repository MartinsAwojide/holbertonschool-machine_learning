...
def convolve(images, kernels, padding='same', stride=(1, 1)):
    ...
    Args:
        ...
        kernels: is a numpy.ndarray with shape (kh, kw) containing the
    ...
    kernel_h, kernel_w, _, nc = kernels.shape
    ...
    convolution = np.zeros((m, conv_h, conv_w, nc))
    ...
    for height in range(conv_h):
        for width in range(conv_w):
            for n in range(nc):
                ...
                convolution[:, height, width, n] = (np.sum(aux * kernel,
                                                           axis=(1, 2, 3)))
    return convolution
